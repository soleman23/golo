...
export default function PayoutsPage() {
  ...
  const activeLiveRoundId = useLiveRoundStore((s) => s.liveRoundId)
  ...
  useEffect(() => {
    if (!authEnabled || !activeLiveRoundId) return undefined
    let cancelled = false
    fetchBettingGate(activeLiveRoundId).then((gate) => {
      if (!cancelled) setBettingGate(gate)
    })
    return () => { cancelled = true }
  }, [authEnabled, activeLiveRoundId])
      ...
      <div style={S.column}>
        ...
        <div className="golo-scroll" style={S.scroll}>
          ...
          {bettingGate?.hasTerms && !bettingGate.active && (
            <button
              type="button"
              onClick={() => activeLiveRoundId && navigate(`/betting/${activeLiveRoundId}`)}
              style={S.betWarn}
            >
