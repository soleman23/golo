...
- **Phase 4 · Betting acceptance**
  ...
  - **Payout gate (✅ BUILT 2026-07-15):** `fetchBettingGate(roundId)` + a warning banner on PayoutsPage listing anyone still pending/declined ("the bet isn't binding for them"), tap → `/betting/:roundId`.
  - **Leftovers (✅ BUILT 2026-07-15):** "Review betting terms & acceptance" link in the ScoringPage Active Bets sheet (→ `/betting/:roundId`); organizer **Re-lock terms · new version** button on BettingReviewPage (re-finalizes from current round state, superseding + re-pending everyone — the material-change path).
  - **Status:** Phase 4 complete. 0025 applied; foundation + UI + payout gate + leftovers committed on `feat/notifications-inbox` (PR #1).
- **Phase 5 · Payment lifecycle**
  - **Foundation (✅ CODE BUILT 2026-07-15, not applied):** `0026_payment_requests.sql` — `payment_requests` (payer/recipient user + slot ids, amount, frozen `calculation_snapshot`, `pending→viewed→marked_sent→confirmed` + `disputed`/`cancelled`, unique per round+payer+recipient so re-completing never duplicates). RLS: round members read; writes only via RPCs. RPCs: `create_payment_requests(round, settlements, snapshot)` (scorer/owner, maps round-slot ids → member user ids, notifies payer), `mark_payment_sent` (payer only → notifies recipient), `confirm_payment_received` (recipient only → notifies payer), `dispute_payment` (either), `mark_payment_viewed`, `cancel_payment` (organizer). Privacy-safe notify text (no amounts); `payment_*` types → category `payments` (push on); Realtime on `payment_requests`. Client: `src/lib/db/payments.js`.
  - **UI (NEXT):** a `/payments/:roundId` screen (payer: amount, recipient, breakdown, external Venmo link, Mark as sent, Dispute; recipient: Confirm received, Dispute) + status beside every settlement on PayoutsPage; create requests on **Complete** (locked results). Venmo opens externally but GoLo still waits for both confirmations.
